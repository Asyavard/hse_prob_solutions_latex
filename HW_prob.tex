...
$F_{X|Y} =$ 
 \begin{cases}
   \frac{f_{X,Y}(x,y)}{f_Y(y)}=\frac{6x \cdot 0.25}{3 \cdot 0.25}=2x, &\text{$x \in [0;1]$}\\
   0, &\text{иначе}\\
 \end{cases}
 \end{center}
...
\[
\P\qty(\grande\frac{\hat\sigma^2(n-1)}{\chi^2_{n-1;1-\frac{\alpha}{2}}}<\sigma^2<\frac{\hat\sigma^2(n-1)}{\chi^2_{n-1;\frac{\alpha}{2}}})=1-\alpha
\]
\[
...
\item
\textbf{Контрольная работа №3 (2013-2014гг). Задача 2.}

$n_1 = 85, n_2 = 100$

$\hat p_1 = \frac{1}{3+1}=\frac{1}{4}, \hat p_2 = \frac{1}{1+1}=\frac{1}{2}$
...
$H_0$ не отвергается в интервале $(-2.05; 2.05)$. Так как расчётное значение не входит в этот интервал \Rightarrow $H_0$ отвергается \Rightarrow \text{преподаватели предъявляют разные требования}$

\[
p-value=2\P(Z >\abs{Z_\p}) = 2\P(Z>3.483) = 2(1-\P(Z\leqslant3.483))=2(1-0.9997)=0.0006
\]


\item
\textbf{Контрольная работа №3 (2009-2010гг). Задача 1.}

\[
\overline{X}=\frac{-1.5+2.6+1.2-2.1+0.1+0.9}{6}=0.2
\]
\[
\hat\sigma^2=\frac{\sum {(X_i-\overline X)}^2}{n}=\frac{15.44}{6}=2.573 \text{ неисправленная}
\]
\[
\hat\sigma^2=\frac{\sum {(X_i-\overline X)}^2}{n-1}=\frac{15.44}{5}=3.088 \text{ исправленная}
\]
...
\item
\textbf{Контрольная работа №3 (2009-2010гг). Задача 3.}
...
\]
\[
\Var(\hat X_i)=\frac{\sigma_i^2}{n_i},
\] 

где i – номер страты, S – количество страт\\
...
\[
\E(\hat\theta_\textit{MM})=\E(2\overline{X})=2\E\qty(\grande\frac{\sum_{i=1}^{n} X_i }{n})=2\Е(X_i) = 2\cdot \frac{\theta}{2} = \theta
\]

следовательно, найденная оценка является несмещенной}\\

Оценка является состоятельной, если она несмещенная (или асимптотически несмещенная) и 
\[
\lim_{n\to\infty}\Var(\hat{\theta_n})=0
\]
\[
...
Оценка считается эффективнее, если ее дисперсия меньше. Так как $X_1, X_2,\ldots, X_n$ – независимые одинаково распределенные случайные величины
...
\[
X_i = X_n = \max(X_1,\ldots, X_n)
\]
\[
\Var(T)=\Var\qty(\tall\frac{n+1}{n}X_i)=\qty(\tall{\frac{n+1}{n}})^2\Var (X_i)=\qty(\grande{\frac{n+1}{n}})^2\cdot\frac{\theta^2}{12}
\]

Сравним дисперсии оценок:

\begin{align*}
\Var(\hat\theta_\textit{MM})-\Var(T)=\frac{\theta^2}{3n}-\qty(\tall{\frac{n+1}{n}})^2\cdot\frac{\theta^2}{12} = (4n-(n+1)^2 )\cdot\frac{\theta^2}{12n^2}=\\=(4n-n^2-2n-1)\cdot\frac{\theta^2}{12n^2}=(n^2-2n+1)-\frac{\theta^2}{12n^2}=(n-1)^2-\frac{\theta^2}{12n^2}<0
\end{align*}

следовательно 

\[
\Var(\hat\theta_\textit{MM})<\Var(T))
\]

а значит $\hat\theta_\textit{MM}$ эффективнее.



\item
\textbf{Контрольная работа №1 (2008-2009 гг). Задача 3.}
